...
import { ImageIcon, Minus, Plus, Sparkle, Sparkles, Wand2 } from 'lucide-react'
import { Input } from '@/components/ui/input'
import { BarLoader } from 'react-spinners'
import { generateBlogContent, improveContent } from '../actions/gemini'
...
const PostEditorContent = ({form ,setQuillRef , onImageUpload}) => {
 ...
 const handleAI=async(type , improvementType=null)=>{
      ...
      if(type==="generate"){
           ...
           if(content && content!=="<p><br></p>" && !window.confirm("This will replace your existing content. Continue?"))
            return ;
  ...
  return (
    ...
    <main className="max-w-4xl mx-auto px-6 py-8">
       ...
       (<button
           ...
           >
              <ImageIcon
                className="h-12 w-12 text-slate-400 group-hover:text-slate-300"
              />
           ...
           <div>
             ...
             >
                <Wand2 className="h-4 w-4 mr-2"/>Generate Content with AI
             </Button>)
             :
             ...
             {!hasTitle && (
                <p className='text-xs text-slate-400 w-full pt-2'>Add a title to enable AI content generation</p>
              )
             }
